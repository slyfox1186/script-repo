...
impl MemoryProfiler {
    ...
    async fn record_system_baseline(&self) -> GccResult<()> {
        ...
        debug!(
            "Recorded system memory baseline: {} MB available",
            available_mb
        );
        Ok(())
    }
    ...
    pub async fn estimate_memory_usage(
        ...
        // Find similar builds
        let similar = self
            .find_similar_builds(&profiles, gcc_version, config_hash)
            .await;

        if !similar.is_empty() {
            let avg_memory: u64 =
                similar.iter().map(|p| p.peak_memory_mb).sum::<u64>() / similar.len() as u64;
    ...
    fn calculate_similarity(
        ...
        if profile.gcc_version == gcc_version.to_string() {
            ...
            .gcc_version
            .starts_with(&gcc_version.major.to_string())
        {
            score += 0.3;
        }
    ...
    pub async fn get_stats(&self) -> ProfilerStats {
        ...
        ProfilerStats {
            ...
            oldest_profile: profiles
                .build_profiles
                .values()
                .map(|p| p.last_updated)
                .min(),
            newest_profile: profiles
                .build_profiles
                .values()
                .map(|p| p.last_updated)
                .max(),
...
impl BuildProfileSession {
    ...
    pub async fn finish(mut self) -> GccResult<()> {
        ...
        // Save profile
        {
            ...
            profiles
                .build_profiles
                .insert(self.profile_key.clone(), profile);
        }
