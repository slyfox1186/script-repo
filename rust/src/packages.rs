...
impl PackageManager {
    ...
    fn init_package_groups() -> HashMap<String, Vec<String>> {
        ...
        groups.insert("build_optimization".to_string(), vec!["ccache".to_string()]);
        ...
        groups.insert(
            "multilib_i386".to_string(),
            vec!["libc6-dev-i386".to_string()],
        );
    ...
    pub async fn check_packages_installed(&self, packages: &[String]) -> GccResult<PackageStatus> {
        ...
        debug!(
            "Checking installation status for {} packages",
            packages.len()
        );
        ...
        let dpkg_output = match self
            .command_executor
            .execute_with_output("dpkg-query", dpkg_args)
            .await
        {
            Ok(output) => output,
            Err(_) => {
    ...
    pub async fn install_packages(&self, packages: &[String]) -> GccResult<()> {
        ...
        self.command_executor
            ...
            .await
            .map_err(|e| {
                GccBuildError::package_manager(format!("Failed to update package lists: {}", e))
            })?;
        ...
        executor_with_env
            ...
            .await
            .map_err(|e| {
                GccBuildError::package_manager(format!("Failed to install packages: {}", e))
            })?;
    ...
    pub async fn get_package_info(&self, package: &str) -> GccResult<PackageInfo> {
        let output = self
            .command_executor
            .execute_with_output("apt-cache", ["show", package])
            .await?;
    ...
    pub async fn remove_packages(&self, packages: &[String]) -> GccResult<()> {
        ...
        self.command_executor
            .execute("sudo", apt_args)
            .await
            .map_err(|e| {
                GccBuildError::package_manager(format!("Failed to remove packages: {}", e))
            })?;
    ...
    pub async fn update_package_cache(&self) -> GccResult<()> {
        ...
        self.command_executor
            ...
            .await
            .map_err(|e| {
                GccBuildError::package_manager(format!("Failed to update package cache: {}", e))
            })?;
    ...
    pub async fn upgrade_packages(&self) -> GccResult<()> {
        ...
        self.command_executor
            ...
            .await
            .map_err(|e| {
                GccBuildError::package_manager(format!("Failed to upgrade packages: {}", e))
            })?;
    ...
    pub async fn clean_package_cache(&self) -> GccResult<()> {
        ...
        self.command_executor
            ...
            .await
            .map_err(|e| {
                GccBuildError::package_manager(format!("Failed to clean package cache: {}", e))
            })?;
...
pub async fn install_dependencies(config: &Config) -> GccResult<()> {
    ...
    let package_status = package_manager
    ...
    package_manager
        .install_packages(&package_status.missing)
        .await?;

    // Skip verification in dry-run mode
    if !config.dry_run {
        // Verify installation
        let verification_status = package_manager
            .check_packages_installed(&package_status.missing)
            .await?;

        if !verification_status.missing.is_empty() {
            return Err(GccBuildError::package_manager(format!(
                "Failed to install packages: {}",
                verification_status.missing.join(", ")
            )));
        }
    }
