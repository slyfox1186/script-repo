...
curl_timeout='10'
git_token='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'
...
build_pkgs_fn()
{
    ...
    pkgs=(ant autoconf autogen automake binutils bison build-essential ccache ccdiff checkinstall clang \
          ...
          libtool libtool-bin libyuv-dev libzen-dev libzstd-dev libzzip-dev lsb-base lshw lvm2 lzma-dev make man-db mercurial \
...
if build 'nasm' '2.16.02rc1'; then
    ...
    execute ./configure --prefix="$workspace" --disable-shared --enable-static CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if build 'm4' '1.4.19'; then
    ...
    execute ./configure --prefix="$workspace" --enable-c++ --with-dmalloc --enable-static --disable-shared CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if build 'cmake' "$g_ver" "$packages/$1.done"; then
    ...
    execute ./configure --prefix="$workspace" --parallel="$cpu_threads" --enable-ccache -- -DCMAKE_USE_OPENSSL='OFF' CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if build 'googletest' "$g_ver"; then
    ...
    make_dir 'build'
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_GMOCK='OFF' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'libgav1' 'git'; then
    ...
    execute cmake -B 'libgav1_build' -DCMAKE_INSTALL_PREFIX="$workspace" -DCMAKE_EXPORT_COMPILE_COMMANDS='ON' -DABSL_ENABLE_INSTALL='ON' \
        -DABSL_PROPAGATE_CXX_STD='ON' -DCMAKE_INSTALL_SBINDIR="sbin" -DENABLE_SHARED='OFF' -DBUILD_SHARED_LIBS='OFF' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
...
if build 'svtav1' "$g_ver"; then
    ...
    execute cmake -S . -B 'Build/linux' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_BUILD_TYPE='Release' -G 'Ninja' CXXFLAGS="$CXX_ZEN" ../..
...
if command_exists 'cargo'; then
    ...
    if build 'rav1e' "$g_ver"; then
        ...
        execute cargo cinstall --prefix="$workspace" --library-type='staticlib' --crt-static --release CXXFLAGS="$CXX_ZEN"
...
if $nonfree_and_gpl; then
    ...
    if build 'vid_stab' "$g_ver"; then
        ...
        execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_BUILD_TYPE='Release' \
             -DUSE_OMP='ON' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
        execute ninja "-j$cpu_threads" -C 'build'
        execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'av1' 'd192cdf'; then
    ...
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DCMAKE_INSTALL_LIBDIR="$workspace"/lib \
        ...
        -DENABLE_TESTS='0' "$packages"/av1 -G 'Ninja' CXXFLAGS="$CXX_ZEN" "$packages"/av1
        execute ninja "-j$cpu_threads" -C 'build'
        execute ninja "-j$cpu_threads" -C 'build' install
...
if build "libpng" '1.6.39'; then
    ...
    execute ./configure --prefix="$workspace" --disable-shared --enable-static --enable-unversioned-links \
        --enable-hardware-optimizations LDFLAGS="$LDFLAGS" CPPFLAGS="$CFLAGS" CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
...
if build 'kvazaar' "$g_ver"; then
    ...
    execute ./autogen.sh
    execute ./configure --prefix="$workspace" --enable-static --disable-shared --enable-fast-install='yes' CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if build 'opus' "$g_ver"; then
    ...
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_C_FLAGS_DEBUG='-g' \
        -DCPACK_SOURCE_ZIP='OFF' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'libvorbis' "$g_ver"; then
    ...
    make_dir 'build'
    execute ./autogen.sh
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'libtheora' '1.0'; then
    ...
    execute ./configure --prefix="$workspace" --with-ogg-libraries="$workspace"/lib --with-ogg-includes="$workspace"/include \
        ...
        --disable-oggtest --disable-vorbistest --disable-examples --disable-asm --disable-spec CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if $nonfree_and_gpl; then
    ...
    download "$g_url" "fdk_aac-$g_ver.tar.gz"
        ...
        execute ./configure --prefix="$workspace" --bindir="$workspace"/bin --disable-shared --enable-static --enable-pic \
            CXXFLAGS="$CXX_ZEN -fno-exceptions -fno-rtti"
        execute make "-j$cpu_threads"
        execute make install
...
if build 'libwebp' 'git'; then
    ...
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DENABLE_SHARED='OFF' -DBUILD_SHARED_LIBS='OFF' -DCMAKE_BUILD_TYPE='Release' \
        ...
        -DWEBP_BUILD_VWEBP='OFF' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
...
if build 'frei0r' "$g_ver"; then
    ...
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DWITHOUT_OPENCV='OFF' \
        ...
        -DWEBP_ENABLE_SWAP_16BIT_CSP='ON' -DENABLE_SHARED='OFF' -DBUILD_SHARED_LIBS='OFF' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'avisynth' "$g_ver"; then
    ...
    make_dir 'build'
    execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'udfread' "$g_ver1"; then
    ...
    execute autoreconf -fi
    execute ./configure --prefix="$workspace" --disable-shared --enable-static --with-pic --with-gnu-ld CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if build 'libbluray' "$g_ver1"; then
    ...
    execute autoreconf -fi
    execute ./configure --prefix="$workspace" --disable-shared --enable-static --without-libxml2 CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if build 'flac' "$g_ver"; then
    ...
    execute cmake -B 'build' -DWITH_STACK_PROTECTOR='ON' -DBUILD_TESTING='OFF' -DBUILD_CXXLIBS='ON' -DWITH_FORTIFY_SOURCE='ON' -DCMAKE_BUILD_TYPE='Release' \
        ...
        -DBUILD_EXAMPLES='OFF' -G 'Ninja' -Wno-dev CXXFLAGS="$CXX_ZEN"
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'MediaInfoCLI' "$g_ver"; then
    ...
    execute ./autogen.sh
    execute ./configure --prefix="$workspace" --enable-staticlibs CXXFLAGS="$CXX_NAT"
    execute make "-j$cpu_threads"
    execute make install
...
if build 'fontconfig' "$g_ver"; then
    ...
    execute ./autogen.sh
    execute ./configure --prefix="$workspace" --sysconfdir="$workspace"/etc --mandir="$workspace"/share/man CXXFLAGS="$CXX_ZEN"
    execute make "-j$cpu_threads"
    execute make install
...
if $nonfree_and_gpl; then
    ...
    if build 'srt' '1.5.1'; then
        ...
        execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DENABLE_APPS='OFF' -DENABLE_SHARED='OFF' -DENABLE_STATIC='ON' \
            -DPKG_CONFIG_EXECUTABLE='/usr/bin' -DENABLE_DEBUG='OFF' -G 'Ninja' CXXFLAGS="$CXX_ZEN"
...
if build 'gpac' "$g_ver"; then
    ...
    execute ./configure --prefix="$workspace" --cc='gcc-12' --cxx='g++-12' --extra-cflags="$CFLAGS" \
        ...
        --cpu='x86_64' --static-build --static-bin --static-modules --enable-gprof CXXFLAGS="$CXX_ZEN"
...
if build 'opencl' "$g_ver"; then
    ...
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" CXXFLAGS="$CXX_ZEN"
