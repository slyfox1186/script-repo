...
source_flags_fn()
{
    ...
    LDFLAGS="-Wl,--as-needed -L${workspace}/lib64 -L${workspace}/lib -L${workspace}/lib/x86_64-linux-gnu -L/usr/local/lib/x86_64-linux-gnu"
    LDFLAGS+=" -L${install_dir}/lib64 -L${install_dir}/lib -L/usr/lib/x86_64-linux-gnu -L/usr/lib64 -L/usr/lib -L/lib64 -L/lib"
...
git_1_fn()
{
    ...
    # LOOP THE VARIABLE G_VER TO MAKE SURE WE DON'T USE A RELEASE CANDIDATE VERSION AKA 'RC'
    get_str_end='-rc[0-9]'
    while [[ ${g_ver} =~ ${get_str_end}$ ]]
    do
        if curl_cmd="$(curl -A "${user_agent}" -m "${curl_timeout}" -sSL "https://gitlab.kitware.com/api/v4/projects/${gitlab_repo}/repository/${github_url}" | jq -r '.[$cnt].name')"; then
            g_ver="${curl_cmd#v}"
            ((cnt++))
        fi
    done
...
git_2_fn()
{
    ...
    # LOOP THE VARIABLE G_VER TO MAKE SURE WE DON'T USE ANY RELEASE CANDIDATES AKA 'RC'. STABLE RELEASES ONLY.
    get_str_end='-rc[0-9]'
    while [[ ${g_ver} =~ ${get_str_end}$ ]]
    do
        if curl_cmd="$(curl -A "${user_agent}" -m "${curl_timeout}" -sSL "https://code.videolan.org/api/v4/projects/${repo}/repository/${url}" | jq -r ".[$cnt].name")"; then
            g_ver="${curl_cmd#v}"
        fi
        ((cnt++))
    done
...
git_3_fn()
{
    ...
    while [[ ${g_ver} =~ ${get_str_end}$ ]]
    do
        if curl_cmd="$(curl -A "${user_agent}" -m "${curl_timeout}" -sSL "https://gitlab.com/api/v4/projects/${repo}/repository/${url}" | jq -r ".[$cnt].name")"; then
            g_ver="${curl_cmd#v}"
        fi
        ((cnt++))
    done
...
git_4_fn()
{
    ...
    while [[ ${g_ver} =~ ${get_str_end}$ ]]
    do
        if curl_cmd="$(curl -A "${user_agent}" -m "${curl_timeout}" -sSL "https://gitlab.freedesktop.org/api/v4/projects/${repo}/repository/tags" | jq -r ".[$cnt].name")"; then
            g_ver="${curl_cmd#v}"
        fi
        ((cnt++))
    done
...
git_5_fn()
{
    ...
    while [[ ${g_ver} =~ ${get_str_end}$ ]]
    do
        if curl_cmd="$(curl -A "${user_agent}" -m "${curl_timeout}" -sSL "https://gitlab.gnome.org/api/v4/projects/${repo}/repository/tags" | jq -r ".[$cnt].name")"; then
            g_ver="${curl_cmd#v}"
        fi
        ((cnt++))
    done
...
git_6_fn()
{
    ...
    while [[ ${g_ver} =~ ${get_str_end}$ ]]
    do
        if curl_cmd="$(curl -A "${user_agent}" -m "${curl_timeout}" -sSL "https://salsa.debian.org/api/v4/projects/${repo}/repository/tags" | jq -r ".[$cnt].name")"; then
            g_ver="${curl_cmd#v}"
        fi
        ((cnt++))
    done
...
cuda_download_fn()
{
    ...
    case "${choice}" in
        1)
            wget --show progress -U "${user_agent}" -cqO "${packages}/nvidia-cuda/cuda-buster-${cuda_latest_ver}.deb" "${cuda_url}/local_installers/cuda-repo-debian10-12-3-local_12.3.1-545.23.08-1_amd64.deb"
            ...
            ;;
        2)
            wget --show progress -U "${user_agent}" -cqO "${packages}/nvidia-cuda/cuda-bullseye-${cuda_latest_ver}.deb" "${cuda_url}/local_installers/cuda-repo-debian11-12-3-local_12.3.1-545.23.08-1_amd64.deb"
            ...
            ;;
        3)
            wget --show progress -U "${user_agent}" -cqO "${packages}/nvidia-cuda/cuda-bookworm-${cuda_latest_ver}.deb" "${cuda_url}/local_installers/cuda-repo-debian12-12-3-local_12.3.1-545.23.08-1_amd64.deb"
        ...
        4)
            ...
            wget --show progress -U "${user_agent}" -cqO "${packages}/nvidia-cuda/cuda-focal-${cuda_latest_ver}.deb" "${cuda_url}/local_installers/cuda-repo-ubuntu2004-12-3-local_12.3.1-545.23.08-1_amd64.deb"
        ...
        5)
            ...
            wget --show progress -U "${user_agent}" -cqO "${packages}/nvidia-cuda/cuda-jammy-${cuda_latest_ver}.deb" "${cuda_url}/local_installers/cuda-repo-ubuntu2204-12-3-local_12.3.1-545.23.08-1_amd64.deb"
        ...
        6)
            ...
            wget --show progress -U "${user_agent}" -cqO "${packages}/nvidia-cuda/cuda-wsl-${cuda_latest_ver}.deb" "${cuda_url}/local_installers/cuda-repo-wsl-ubuntu-12-3-local_12.3.1-1_amd64.deb"
...
pkgs_fn()
{
    ...
    pkgs=("${1}" "${libcpp_pkg}" "${libunwind_pkg}" "${libcppabi_pkg}" ant apt asciidoc autoconf autoconf-archive
          ...
          libmusicbrainz5-dev libmysofa-dev libnuma-dev libopencore-amrnb-dev libopencore-amrwb-dev libopencv-dev
          libopenjp2-7-dev libopenmpt-dev libopus-dev libpango1.0-dev libperl-dev libpstoedit-dev libpulse-dev librabbitmq-dev
          libraqm-dev libraw-dev librsvg2-dev librubberband-dev librust-gstreamer-base-sys-dev libshine-dev libsmbclient-dev
          libsnappy-dev libsndfile1-dev libsndio-dev libopencv-dev libsoxr-dev libspeex-dev libsystemd-dev libsqlite3-dev
          libsrt-gnutls-dev libssh-dev libssl-dev libsuitesparseconfig5 libtalloc-dev libtheora-dev libticonv-dev libtool
          libtool-bin libtwolame-dev libudev-dev libumfpack5 libv4l-dev libva-dev libvdpau-dev libvidstab-dev libvlccore-dev
          libvo-amrwbenc-dev libvpx-dev libx11-dev libx264-dev libxcursor-dev libxext-dev libxfixes-dev libxi-dev libxkbcommon-dev
          libxrandr-dev libxss-dev libudev-dev libxvidcore-dev libyuv-dev libzmq3-dev libzstd-dev libzvbi-dev libzzip-dev llvm lshw
          lzma-dev m4 meson nasm ninja-build pandoc python3 python3-pip ragel re2c scons sudo texi2html texinfo tk-dev unzip valgrind
          wget xmlto zlib1g-dev mesa-utils)
...
ubuntu_os_ver_fn()
{
    ...
    case "${VER}" in
        23.10|23.04|22.10)      pkgs_fn "${1} ${ubuntu_common_pkgs} ${lunar_kenetic_pkgs} ${jammy_pkgs}";;
...
if build 'decklink' '12.7'; then
    download 'https://github.com/slyfox1186/script-repo/raw/main/library-files/Blackmagic-DeckLink-SDK-12.7.tar.xz' 'decklink-12.7.tar.xz'
    for files in *.{h,cpp}
    do
...
printf "\n%s\n%s\n"                                 \
    'Building nv-codec-headers - version 12.0.16.1' \
    '========================================================'
