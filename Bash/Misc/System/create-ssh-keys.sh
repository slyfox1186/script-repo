...
# Prompt for adding a password to the SSH key
read -p "Do you want to add a password to the SSH key? (y/n) " add_password
if [[ "$add_password" == "y" ]]; then
    read -s -p "Enter the password for the SSH key: " key_password
    echo
    read -s -p "Confirm the password: " key_password_confirm
    echo
    if [[ "$key_password" != "$key_password_confirm" ]]; then
        error_exit "Passwords do not match. Exiting."
    fi
else
    key_password=""
fi
...
if [[ ! -f "$private_key" || ! -f "$public_key" ]]; then
    ...
    if [[ "$create_keys" == "y" ]]; then
        ssh-keygen -t rsa -b "$key_size" -f "$private_key" -N "$key_password" -C "$email_address" || error_exit "Failed to generate SSH key pair"
...
# Remove duplicate entries in authorized_keys file
sort "$authorized_keys" | uniq > "$authorized_keys.tmp" && mv "$authorized_keys.tmp" "$authorized_keys" || error_exit "Failed to remove duplicate entries from authorized_keys file"
log "Removed duplicate entries from authorized_keys file"
