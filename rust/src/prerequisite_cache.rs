...
impl PrerequisiteCache {
        ...
        if state_file.exists() {
            ...
            if let Ok(state) = serde_json::from_str::<CacheState>(&content) {
                *self.state.write().await = state;
                info!(
                    "Loaded prerequisite cache with {} entries",
    ...
    async fn download_prerequisite(&self, name: &str, version: &str, url: &str) -> GccResult<()> {
        ...
        for attempt in 1..=retry_count {
            match self
                .command_executor
                .execute_with_output("curl", ["-fSL", "-o", tarball_path.to_str().unwrap(), url])
                .await
            {
                Ok(_) => break,
    ...
    async fn link_prerequisite(
        ...
        let prereq = state
            ...
            .ok_or_else(|| {
                GccBuildError::configuration(format!(
                    "Prerequisite {} {} not found in cache",
                    name, version
                ))
            })?;
        ...
        #[cfg(unix)]
        {
            ...
            symlink(&prereq.extracted_path, &link_path).map_err(|e| {
    ...
    pub async fn get_stats(&self) -> CacheStats {
        let state = self.state.read().await;
        let total_size: u64 = state.prerequisites.iter().map(|p| p.size_bytes).sum();

        CacheStats {
            total_prerequisites: state.prerequisites.len(),
            total_size_mb: total_size / 1_000_000,
            unique_versions: state
                .prerequisites
                .iter()
                .map(|p| &p.name)
    ...
    pub async fn cleanup_old(&self, keep_latest: usize) -> GccResult<()> {
        ...
        info!(
            "Cleaned up prerequisite cache, kept {} entries",
