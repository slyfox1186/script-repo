...
impl CommandExecutor {
        ...
        S: AsRef<OsStr>,
    {
        ...
        Err(last_error
            .unwrap_or_else(|| GccBuildError::command_failed(format!("{} with args", program), -1)))
    }
        ...
        S: AsRef<OsStr>,
    {
        match timeout(timeout_duration, self.execute(program, args)).await {
            Ok(result) => result,
            Err(_) => Err(GccBuildError::network_timeout(format!(
                "Command timed out: {}",
                program
            ))),
        }
    ...
    pub async fn execute_as<I, S>(&self, user: &str, program: &str, args: I) -> GccResult<()>
    where
        I: IntoIterator<Item = S>,
        S: AsRef<OsStr>,
    {
        match user {
            "sudo" | "root" => {
                ...
                self.execute("sudo", sudo_args.iter().map(|s| s.as_str()))
                    .await
            }
            "user" | "" => self.execute(program, args).await,
            _ => {
                let args_vec: Vec<_> = args
                    .into_iter()
                    .map(|s| s.as_ref().to_string_lossy().to_string())
        ...
        S: AsRef<OsStr>,
    {
        ...
        let is_version_resolution = program == "bash"
            && args.iter().any(|arg| {
                arg.as_ref()
                    .to_string_lossy()
                    .contains("curl -fsSL https://ftp.gnu.org/gnu/gcc/")
            });
                ...
                Ok(_) => {
                    ...
                    if let Ok(metadata) = output_path.metadata() {
                        ...
                        if file_size > 10_000_000 {
                            // At least 10MB for GCC source
                            info!(
                                "✅ Download completed successfully ({:.1} MB)",
                            ...
                            last_error = Some(GccBuildError::download(
                                url.to_string(),
                                format!(
                                    "Downloaded file too small: {:.1} MB",
                                    file_size as f64 / 1_000_000.0
                                ),
                            ));
                        }
