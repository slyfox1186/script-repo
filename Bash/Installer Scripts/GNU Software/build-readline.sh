...
# Github: https://github.com/slyfox1186/script-repo/blob/main/Bash/Installer%20Scripts/GNU%20Software/build-readline.sh
...
required_packages() {
    ...
    local pkg
    pkgs=(
        autoconf autoconf-archive autogen automake autopoint autotools-dev
        build-essential bzip2 ccache curl git libaudit-dev libintl-perl 
        librust-polling-dev libtool lzip m4 pkg-config valgrind zlib1g-dev
    )
...
set_compiler_flags() {
    CC="$compiler"
    CXX="$compiler++"
    CFLAGS="-O2 -fPIC -fPIE -pipe -march=native" 
    CXXFLAGS="$CFLAGS"
...
configure_build() {
    ...
    autoreconf -fi
    cd build || exit 1
    ../configure --prefix="$install_dir/$archive_name" --enable-multibyte || fail "Failed to execute: configure. Line: $LINENO"
}
...
uninstall_readline() {
    local readline_dir
    readline_dir="$install_dir/$archive_name"
    if [[ -d "$readline_dir" ]]; then
        log "Uninstalling $prog_name from $readline_dir"
        sudo rm -rf "$readline_dir"
