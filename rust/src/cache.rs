...
impl VersionCache {
    ...
    pub async fn persist(&self, path: &std::path::Path) -> GccResult<()> {
        ...
        tokio::fs::write(path, json).await.map_err(|e| {
            GccBuildError::file_operation("write cache", path.display().to_string(), e.to_string())
        })?;
    ...
    pub async fn load(&self, path: &std::path::Path) -> GccResult<()> {
        ...
        let json = tokio::fs::read_to_string(path).await.map_err(|e| {
            GccBuildError::file_operation("read cache", path.display().to_string(), e.to_string())
        })?;

        let persisted: PersistedCache = serde_json::from_str(&json).map_err(|e| {
            GccBuildError::file_operation(
                "deserialize cache",
                path.display().to_string(),
                e.to_string(),
