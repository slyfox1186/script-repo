#!/usr/bin/env python3
"""
TurboQuant llama.cpp Automated Builder (Native/Generic)
=======================================================
Automates the full installation and build of TurboQuant KV cache compression
for llama.cpp from TheTom's fork.

Uses -DGGML_NATIVE=ON to auto-detect CPU features at build time.
No hardcoded x86 SIMD flags — works on any CPU that supports the detected
instruction sets.

Target system:
  - Arch Linux (also supports Debian/Ubuntu)
  - Any modern CPU (GGML_NATIVE auto-detects SIMD)
...
First-time setup (if conda env does not exist yet):
  ...
  python3 build_turboquant_native.py

Usage:
  python3 build_turboquant_native.py              # Full install + build
  python3 build_turboquant_native.py --deps-only  # Install dependencies only
  python3 build_turboquant_native.py --build-only # Build only (skip deps + pip)
  python3 build_turboquant_native.py --clean      # Nuke build dir and rebuild
  python3 build_turboquant_native.py --run        # Build + launch server
  python3 build_turboquant_native.py --run --model /path/to/model.gguf
  python3 build_turboquant_native.py --uninstall  # Remove from /usr/local/bin
"""
...
def configure_and_build(repo_dir: Path, info: SystemInfo, clean: bool = False):
    """Run cmake configure and build with native CPU detection."""
    log = logging.getLogger("turboquant-builder")
    ...
    cuda_arch = info.gpu_arch
    if not cuda_arch:
        log.warning("Could not detect GPU compute capability — omitting CMAKE_CUDA_ARCHITECTURES")
        log.warning("CMake will attempt auto-detection at configure time")
    ...
    cmake_args = [
        ...
        # ─ Native CPU detection (auto-detects SIMD: SSE4.2, AVX, AVX2, AVX512, etc.) ─
        "-DGGML_NATIVE=ON",
    ...
    # Only set CUDA arch if detected — otherwise let CMake auto-detect
    if cuda_arch:
    ...
    log.info("Configuring with CMake (native CPU detection)...")
...
def write_manifest(install_dir: Path, info: SystemInfo, build_dir: Path):
    ...
    manifest = {
        ...
        "system": {
            ...
            "gpu": info.gpu_name,
            "gpu_arch": f"sm_{info.gpu_arch}" if info.gpu_arch else "auto-detected",
        ...
        "cmake_flags": {
            ...
            "CMAKE_CUDA_ARCHITECTURES": info.gpu_arch or "auto-detected",
...
def main():
    parser = argparse.ArgumentParser(
        description="Automated TurboQuant llama.cpp builder (native CPU detection)",
...
Examples:
  ...
  %(prog)s --uninstall                        # Remove from /usr/local/bin
    ...
    log.info("=" * 60)
    log.info("  TurboQuant llama.cpp — Automated Builder (Native)")
    log.info(f"  CPU SIMD: auto-detected via GGML_NATIVE=ON")
    ...
    log.info("")
    log.info("=" * 60)
    log.info("  BUILD + INSTALL COMPLETE (Native)")
    log.info("=" * 60)
    log.info(f"  CPU SIMD:      auto-detected (GGML_NATIVE=ON)")
