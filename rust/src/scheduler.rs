...
impl BuildScheduler {
    pub fn new(config: &Config) -> Self {
        ...
        println!("─────────────────────────────────────────────────────────────────");
        println!(
            "  • Total system RAM: {:.1} GB ({} MB)",
            config.system_info.ram_mb as f64 / 1024.0,
            config.system_info.ram_mb
        );
        println!(
            "  • RAM per GCC build: ~{:.1} GB ({} MB)",
            ram_per_build as f64 / 1024.0,
            ram_per_build
        );
        println!("  • CPU cores available: {}", config.system_info.cpu_cores);
        println!(
            "  • Max parallel builds: {} (based on available resources)",
            max_concurrent
        );
    ...
    pub async fn acquire_build_slot(&self, version: &GccVersion) -> GccResult<BuildSlot> {
        ...
        if available_ram < self.ram_per_build {
            warn!(
                "⚠️ Low RAM detected ({} MB). Waiting for builds to complete...",
                available_ram
            );
        ...
        info!(
            "🎯 Build slot acquired for GCC {} (RAM: {} MB allocated)",
            version, self.ram_per_build
        );
    ...
    pub fn update_build_phase(&self, version: &GccVersion, phase: BuildPhase) {
            ...
            .find(|b| &b.version == version)
        {
            ...
            debug!(
                "Build phase updated for GCC {}: {:?} -> {:?}",
                version, old_phase, phase
            );
            info!(
                "🔄 Phase transition for GCC {}: {:?} (CPU: {:.1}%, Mem: {:.1}x, I/O: {:.1}%)",
                version,
                phase,
                self.phase_profiles
                    .get(&phase)
                    .map(|p| p.cpu_intensity * 100.0)
                    .unwrap_or(0.0),
                self.phase_profiles
                    .get(&phase)
        ...
        loop {
            ...
            if start.elapsed() > max_wait {
                return Err(GccBuildError::resource_exhausted(
                    "RAM",
                    format!(
                        "Waited {} seconds but insufficient RAM available",
                        max_wait.as_secs()
                    ),
                ));
            }
    ...
    pub async fn calculate_optimal_jobs(
        &self,
        version: &GccVersion,
        base_jobs: Option<usize>,
    ) -> usize {
        let state = self.system_state.lock().unwrap();
    ...
    pub fn get_statistics(&self) -> SchedulerStatistics {
        ...
        SchedulerStatistics {
            ...
            average_build_time: if !state.completed_builds.is_empty() {
                let total_time: Duration = state.completed_builds.iter().map(|b| b.duration).sum();
                Some(total_time / state.completed_builds.len() as u32)
            } else {
                None
            },
            peak_ram_usage: state
                .completed_builds
                .iter()
                .map(|b| b.peak_ram_mb)
                .max()
    ...
    fn detect_cpu_topology() -> CpuTopology {
        ...
        let physical_cores = if let Ok(output) = std::process::Command::new("lscpu").output() {
    ...
    fn determine_scheduling_strategy(
        config: &Config,
    ...
    fn adjust_cpu_allocation(
        ...
        debug!(
            "Adjusting CPU allocation for GCC {} in {:?} phase: intensity={:.1}%",
            version,
            phase,
            profile.cpu_intensity * 100.0
        );
    ...
    pub fn update_system_load(&mut self, current_load: f64) {
        ...
        debug!(
            "System load updated: {:.2} (target: {:.2})",
...
impl BuildSlot {
    ...
    pub fn complete(mut self, success: bool, peak_ram_mb: Option<u64>) {
        ...
        // Update system state
        {
            ...
            // Release RAM
            state.available_ram_mb = state
                .available_ram_mb
                .saturating_add(self.scheduler.ram_per_build);
        }

        info!(
            "🏁 Build slot released for GCC {} (duration: {:?}, success: {})",
    ...
    fn drop(&mut self) {
        if self.permit.is_some() {
            ...
            state.available_ram_mb = state
                ...
                .saturating_add(self.scheduler.ram_per_build);
            warn!(
                "Build slot for GCC {} dropped without completion",
                self.version
            );
        }
    }
