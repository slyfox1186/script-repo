...
random_dir="$(mktemp -d)"
url='https://raw.githubusercontent.com/slyfox1186/script-repo/main/Bash/Arch%20Linux%20Scripts/arch-scripts.txt'
...
if ! sudo packman -Pk | grep -o wget &>/dev/null; then
    sudo packman -Sq --needed --noconfirm wget
    clear
fi
