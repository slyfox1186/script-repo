...
impl FileOperations {
    ...
    pub fn extract_archive(
        ...
        match extension {
            "xz" if stem.ends_with(".tar") => {
                ...
                cmd.arg("-Jxf").arg(archive_path).arg("-C").arg(destination);
    ...
    pub fn create_symlink(&self, target: &Path, link_path: &Path, force: bool) -> GccResult<()> {
        if self.dry_run {
            info!(
    ...
    pub fn create_symlink_sudo(
        ...
        match self.create_symlink(target, link_path, force) {
            ...
            Err(e) => {
                debug!(
                    "Failed to create symlink without sudo: {}, trying with sudo",
                    e
                );
            }
        }
    ...
    pub fn copy_file_with_progress(&self, source: &Path, destination: &Path) -> GccResult<()> {
