...
impl ArtifactCache {
    ...
    pub async fn init(&mut self) -> GccResult<()> {
        ...
        if index_path.exists() {
            ...
            if let Ok(index) = serde_json::from_str::<CacheIndex>(&content) {
                self.index = index;
                info!(
                    "📦 Loaded artifact cache with {} entries ({:.1} GB)",
    ...
    pub async fn get_artifact(
        ...
        let (install_path, build_time_seconds, should_remove) =
        ...
        if let Some(path) = install_path {
            ...
            info!(
                "🎯 Cache HIT for GCC {} (saved ~{}min build time)",
                gcc_version,
    ...
    async fn calculate_verification_hash(&self, path: &Path) -> GccResult<String> {
        ...
        let key_files = ["bin/gcc", "bin/g++", "lib/gcc", "include"];
    ...
    async fn evict_old_artifacts(&mut self) -> GccResult<()> {
        ...
        info!(
            "✅ Cache cleanup completed ({:.1} GB freed)",
