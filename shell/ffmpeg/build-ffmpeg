...
script_ver='3.10'
progname="${0:2}"
# FFmpeg is sourced from the official github repository
ffmpeg_ver='n6.1-dev'
cuda_ver='12.1'
...
git_1_fn()
{
    ...
    github_repo="${1}"
    github_url="${2}"
    curl_cmd=$(curl -m "${net_timeout}" -Ls "https://api.github.com/repos/${github_repo}/${github_url}?per_page=1")
    if [ "${?}" -eq '0' ]; then
...
git_2_fn()
{
    net_timeout='5'
    videolan_repo="${1}"
    if curl_cmd=$(curl -m "${net_timeout}" -Ls "https://code.videolan.org/api/v4/projects/${videolan_repo}/repository/branches?"); then
...
git_3_fn()
{
    net_timeout='5'
    gitlab_repo="${1}"
    if curl_cmd=$(curl -m "${net_timeout}" -Ls "https://gitlab.com/api/v4/projects/${gitlab_repo}/repository/branches?"); then
...
git_4_fn()
{
    net_timeout='5'
    gitlab_repo="${1}"
    if curl_cmd=$(curl -m "${net_timeout}" -Ls "https://gitlab.com/api/v4/projects/${gitlab_repo}/repository/tags"); then
...
git_5_fn()
{
    net_timeout='5'
    gitlab_repo="${1}"
    if curl_cmd=$(curl -Ss "https://gitlab.freedesktop.org/api/v4/projects/${gitlab_repo}/repository/tags?"); then
        gitlab_ver=$(echo "${curl_cmd}" | jq -r '.[0].name')
    fi
}
...
git_6_fn()
{
    ...
    if curl_cmd="$(curl \
        ...
        gitlab_ver=$(echo "${curl_cmd}" | jq '.development.branch.target' | grep -Eo '[0-9]+[0-9a-z]+[^"]')
    fi
}
...
git_7_fn()
{
    net_timeout='5'
    gitlab_repo="${1}"
    if curl_cmd=$(curl -Ss "https://gitlab.gnome.org/api/v4/projects/${gitlab_repo}/repository/tags?"); then
        gitlab_ver=$(echo "${curl_cmd}" | jq -r '.[0].name')
        gitlab_ver=${gitlab_ver#v}
    fi
}
...
build_pkgs_fn()
{
    ...
    pkgs=(g++ gcc gtk-doc-tools help2man jq libcairo2-dev libcdio-paranoia-dev libcurl4-gnutls-dev libglib2.0-dev libmusicbrainz5-dev libtinyxml2-dev pkg-config ragel)
...
cuda_add_fn()
{
    ...
    pkgs=(autoconf automake build-essential libc6 libc6-dev libnuma1 libnuma-dev texinfo unzip wget)
...
if build 'pkg-config' "${github_ver}"; then
    download "https://pkgconfig.freedesktop.org/releases/${github_ver}.tar.gz" "${github_ver}.tar.gz"
    execute ./configure --silent --prefix="${workspace}" --with-pc-path="${workspace}"/lib/pkgconfig --with-internal-glib
    execute make "-j${cpus}"
    execute make install
    build_done 'pkg-config' "${github_ver}"
fi

git_ver_fn 'yasm/yasm' '1' 'T'
if build 'yasm' "${github_ver}"; then
    download "https://github.com/yasm/yasm/releases/download/v${github_ver}/yasm-${github_ver}.tar.gz" "yasm-${github_ver}.tar.gz"
    execute ./configure --prefix="${workspace}"
    execute make "-j${cpus}"
    execute make install
    build_done 'yasm' "${github_ver}"
fi

if build 'nasm' '2.16.02rc1'; then
    https://github.com/netwide-assembler/nasm/archive/refs/tags/nasm-2.16.02rc1.tar.gz
    download "https://www.nasm.us/pub/nasm/releasebuilds/2.16.02rc1/nasm-2.16.02rc1.tar.xz" "nasm-${github_ver}.tar.gz"
...
if build 'zlib' "${github_ver}"; then
    download "${github_tar}" "zlib-${github_ver}"
    execute ./configure --static --prefix="${workspace}"
    execute make "-j${cpus}"
    execute make install
    build_done 'zlib' "${github_ver}"
fi
...
if ${nonfree}; then
    ...
    if build 'openssl' "${github_ver_ossl}"; then
        download "${github_tar}" "openssl-${github_ver_ossl}.tar.gz"
        ...
        build_done 'openssl' "${github_ver_ossl}"
    fi
...
if build 'cmake' "${github_ver}"; then
    download "${github_tar}" "cmake-${github_ver}.tar.gz"
    execute ./configure --prefix="${workspace}" --parallel="${cpus}" -- -DCMAKE_USE_OPENSSL='OFF'
    execute make "-j${cpus}"
    execute make install
    build_done 'cmake' "${github_ver}"
fi
...
if build 'svtav1' "${gitlab_ver}"; then
    download "https://gitlab.com/AOMediaCodec/SVT-AV1/-/archive/v${gitlab_ver}/SVT-AV1-v${gitlab_ver}.tar.bz2" "SVT-AV1-v${gitlab_ver}.tar.bz2"
...
if ${nonfree}; then
    ...
    if build 'x264' "${videolan_sver}"; then
        download "https://code.videolan.org/videolan/x264/-/archive/${videolan_ver}/x264-${videolan_ver}.tar.gz" "x264-${videolan_sver}.tar.gz"
...
if ${nonfree}; then
    ...
    if build 'x265' "${gitlab_ver:0:7}"; then
        download "https://bitbucket.org/multicoreware/x265_git/get/${gitlab_ver}.tar.bz2" "x265-${gitlab_ver:0:8}.tar.bz2"
        cd "${PWD}/build/linux" || exit 1
...
if ${nonfree}; then
    ...
    if build 'vid_stab' "${github_ver}"; then
        download "https://github.com/georgmartius/vid.stab/archive/refs/tags/v${github_ver}.tar.gz" "vid.stab-${github_ver}.tar.gz"
        execute cmake -DBUILD_SHARED_LIBS='OFF' -DCMAKE_INSTALL_PREFIX="${workspace}" -DUSE_OMP='OFF' -DENABLE_SHARED='OFF' .
        execute make "-j${cpus}"
        execute make install
        build_done 'vid_stab' "${github_ver}"
    fi
...
if build 'zimg' "${github_ver}"; then
    download "${github_tar}" "zimg-${github_ver}.tar.gz"
    execute "${workspace}"/bin/libtoolize -i -f -q
    execute ./autogen.sh --prefix="${workspace}"
    execute ./configure --prefix="${workspace}" --enable-static --disable-shared
    execute make "-j${cpus}"
    execute make install
    build_done 'zimg' "${github_ver}"
fi
...
if command_exists 'python3'; then
    if command_exists 'meson'; then
        ...
        if build 'lv2' "${github_ver}"; then
            download "${github_tar}" "lv2-${github_ver}.tar.gz"
            execute meson setup build --prefix="${workspace}" --buildtype='release' --default-library='static' --libdir="${workspace}"/lib
            execute ninja -C build
            execute ninja -C build install
            build_done 'lv2' "${github_ver}"
        fi
...
if build 'opus' "${github_ver}"; then
    download "${github_tar}" "opus-${github_ver}"
    execute ./autogen.sh
    execute ./configure --prefix="${workspace}" --disable-shared --enable-static
    execute make "-j${cpus}"
    execute make install
    build_done 'opus' "${github_ver}"
fi
cnf_ops+=('--enable-libopus')

git_ver_fn 'xiph/ogg' '1' 'T'
if build 'libogg' "${github_ver}"; then
    download "${github_tar}" "libogg-${github_ver}.tar.gz"
    execute ./autogen.sh
    execute ./configure --prefix="${workspace}" --disable-shared --enable-static
    execute make "-j${cpus}"
    execute make install
    build_done 'libogg' "${github_ver}"
fi

git_ver_fn 'xiph/vorbis' '1' 'T'
if build 'libvorbis' "${github_ver}"; then
    download "${github_tar}" "libvorbis-${github_ver}.tar.gz"
    execute ./autogen.sh
    ...
    execute make "-j${cpus}"
    execute make install
    build_done 'libvorbis' "${github_ver}"
fi
...
if ${nonfree}; then
    ...
    if build 'fdk_aac' "${github_ver}"; then
        download "https://github.com/mstorsjo/fdk-aac/archive/refs/tags/v${github_ver}.tar.gz" "fdk_aac-${github_ver}.tar.gz"
        execute ./autogen.sh
...
if build 'libtiff' "${gitlab_ver}"; then
    download "https://gitlab.com/libtiff/libtiff/-/archive/v${gitlab_ver}/libtiff-v${gitlab_ver}.tar.bz2" "libtiff-v${gitlab_ver}.tar.bz2"
    execute ./autogen.sh
    execute ./configure --prefix="${workspace}" --disable-shared --enable-static
    execute make "-j${cpus}"
    execute make install
...
if build 'libpng' "${github_ver}"; then
    download "${github_tar}" "libpng-${github_ver}.tar.gz"
...
if build 'libxml2' "${gitlab_ver}"; then
    ...
    download "https://gitlab.gnome.org/GNOME/libxml2/-/archive/v${gitlab_ver}/libxml2-v${gitlab_ver}.tar.bz2" "libxml2-v${gitlab_ver}.tar.bz2"
    execute ./autogen.sh
    execute ./configure --prefix="${workspace}" --disable-shared --enable-static
    execute make "-j${cpus}"
    execute make install
...
if build 'ZenLib' "${github_ver}"; then
    download "${github_tar}" "ZenLib-${github_ver}.tar.gz"
    cd "${PWD}"/Project/CMake || exit 1
    execute cmake -DCMAKE_INSTALL_PREFIX="${workspace}" -DCMAKE_INSTALL_LIBDIR='lib' -DCMAKE_INSTALL_BINDIR='bin' \
        -DCMAKE_INSTALL_INCLUDEDIR='include' -DENABLE_SHARED='OFF' -DENABLE_STATIC='ON'
    execute make "-j${cpus}"
    execute make install
    build_done 'ZenLib' "${github_ver}"
fi

git_ver_fn 'MediaArea/MediaInfoLib' '1' 'R'
if build 'MediaInfoLib' "${github_ver}"; then
    download "${github_tar}" "MediaInfoLib-${github_ver}.tar.gz"
    cd "${PWD}"/Project/CMake || exit 1
    execute cmake . -DCMAKE_INSTALL_PREFIX="${workspace}" -DCMAKE_INSTALL_LIBDIR='lib' -DCMAKE_INSTALL_BINDIR='bin' \
    ...
    execute make install
    build_done 'MediaInfoLib' "${github_ver}"
fi

git_ver_fn 'MediaArea/MediaInfo' '1' 'T'
if build 'MediaInfoCLI' "${github_ver}"; then
    download "${github_tar}" "MediaInfoCLI-${github_ver}.tar.gz"
    cd "${PWD}"/Project/GNU/CLI || exit 1
    execute ./autogen.sh
    execute ./configure --prefix="${workspace}" --enable-staticlibs
    build_done 'MediaInfoCLI' "${github_ver}"
fi

if command_exists 'meson'; then
    git_ver_fn 'harfbuzz/harfbuzz' '1' 'R'
    if build 'harfbuzz' "${github_ver}"; then
        download "${github_tar}" "harfbuzz-${github_ver}.tar.gz"
        execute ./autogen.sh
        execute meson setup build --prefix="${workspace}" --buildtype='release' --default-library='static' --libdir="${workspace}"/lib
        execute ./configure --prefix="${workspace}" --disable-shared --enable-static
        execute ninja -C build
        execute ninja -C build install
        build_done 'harfbuzz' "${github_ver}"
    fi
fi
...
if build 'fribidi' "${github_ver}"; then
    download "${github_tar}" "fribidi-${github_ver}.tar.gz"
    execute ./autogen.sh
    make_dir build
    execute meson setup build --prefix="${workspace}" --buildtype='release' --default-library='static' --libdir="${workspace}"/lib
        execute ninja -C build
        execute ninja -C build install
    build_done 'fribidi' "${github_ver}"
fi

git_ver_fn 'libass/libass' '1' 'T'
if build 'libass' "${github_ver}"; then
    download "${github_tar}" "libass-${github_ver}.tar.gz"
    execute ./autogen.sh
    execute ./configure --prefix="${workspace}" --disable-shared --enable-static
    execute make "-j${cpus}"
    execute make install
    build_done 'libass' "${github_ver}"
fi
...
if build 'libsdl' "${github_ver}"; then
    download "${github_tar}" "libsdl-${github_ver}.tar.gz"
    execute ./autogen.sh
    execute ./configure --prefix="${workspace}" --disable-shared --enable-static
    execute make "-j${cpus}"
    execute make install
    build_done 'libsdl' "${github_ver}"
fi

if ${nonfree}; then
    git_ver_fn 'Haivision/srt' '1' 'T'
    if build 'srt' "${github_ver}"; then
        download "${github_tar}" "srt-${github_ver}.tar.gz"
...
if build 'amf' "${github_ver}"; then
    download "${github_tar}" "AMF-${github_ver}.tar.gz"
    execute rm -rf "${workspace}"/include/AMF
    execute mkdir -p "${workspace}"/include/AMF
    execute cp -fr "${packages}"/AMF-"${github_ver}"/amf/public/include/* "${workspace}"/include/AMF/
    build_done 'amf' "${github_ver}"
fi
cnf_ops+=('--enable-amf')

git_ver_fn 'fraunhoferhhi/vvenc' '1' 'T'
if build 'vvenc' "${github_ver}"; then
    download "${github_tar}" "vvenc-${github_ver}.tar.gz"
    ...
    build_done 'vvenc' "${github_ver}"
fi
cnf_ops+=('--enable-nvenc')

git_ver_fn 'fraunhoferhhi/vvdec' '1' 'T'
if build 'vvdec' "${github_ver}"; then
    download "${github_tar}" "vvdec-${github_ver}.tar.gz"
    execute cmake -S . -B build/release-static -DCMAKE_INSTALL_PREFIX="${workspace}" \
        -DVVDEC_ENABLE_LINK_TIME_OPT='OFF' -DCMAKE_VERBOSE_MAKEFILE='OFF' -DCMAKE_BUILD_TYPE='Release'
    execute cmake --build build/release-static -j
    build_done 'vvdec' "${github_ver}"
fi
cnf_ops+=('--enable-nvdec')

if which 'nvcc' &>/dev/null ; then
    git_ver_fn 'FFmpeg/nv-codec-headers' '1' 'T'
    if build 'nv-codec' "${github_ver}"; then
        download "${github_tar}" "nv-codec-${github_ver}.tar.gz"
        execute make PREFIX="${workspace}"
        execute make install PREFIX="${workspace}"
        build_done 'nv-codec' "${github_ver}"
    fi
