...
install_deps() {
    ...
    if command -v apt-get >/dev/null 2>&1; then
        apt-get update
        apt-get install --no-install-recommends autoconf automake binutils bison build-essential ccache curl flex \
                                                   gawk gcc g++ gnat libc6-dev libgmp-dev libmpc-dev libmpfr-dev libtool \
                                                   make patch texinfo zlib1g-dev
    elif command -v dnf >/dev/null 2>&1; then
        dnf install autoconf automake binutils bison ccache curl flex gawk gcc gcc-c++ gmp-devel gnat libtool make mpfr-devel patch texinfo zlib-devel
    elif command -v pacman >/dev/null 2>&1; then
        pacman -S --noconfirm --needed autoconf automake binutils bison ccache curl flex gawk gcc gcc-ada gmp-devel libtool make mpfr patch texinfo zlib
    else
...
create_symlinks() {
    ...
    local TARGET=$(curl -sS 'https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.guess;hb=HEAD' | bash)
    
    # Define the array of files for which symlinks are to be created
    declare -a files=("c++" "gcc" "gcc-ar" "gcc-nm" "gcc-ranlib" "gfortran" "g++")

    # Iterate over each file in the array
    for file in "${files[@]}"; do
        source_file="$install_prefix/gcc-$latest_version/bin/$TARGET-$file"
        # Check if source file exists before creating a symlink
        if [[ -f "$source_file" ]]; then
            # Create a symbolic link in /usr/local/bin/ with the desired name
            ln -sfn "$source_file" "/usr/local/bin/$file"
            log_msg "Created symlink for $file in /usr/local/bin/"
        else
            log_msg "Source file $source_file does not exist, skipping."
        fi
    done
}
