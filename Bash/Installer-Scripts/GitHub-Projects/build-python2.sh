...
##  Github Script: https://github.com/slyfox1186/script-repo/blob/main/Bash/Installer-Scripts/GitHub-Projects/build-python2
...
archive_dir="python2-${python_version}"
archive_url="https://www.python.org/ftp/python/2.7.18/Python-${python_version}.tar.xz"
...
export CC CXX CFLAGS CXXFLAGS CPPFLAGS PATH PKG_CONFIG_PATH
...
if [ ! -f "$working/${archive_name}" ]; then
    curl -Lso "$working/${archive_name}" "${archive_url}"
fi

# Create the output directory

[[ -d "$working/$archive_dir" ]] && sudo rm -fr "$working/$archive_dir"
