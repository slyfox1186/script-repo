...
impl GccConfigGenerator {
    ...
    fn init_base_options() -> HashMap<String, String> {
        ...
        // Let GCC handle ISL detection naturally - don't force --with-isl options
        options.insert(
            "major_version_only".to_string(),
            "--with-gcc-major-version-only".to_string(),
        );
    ...
    fn init_version_features() -> HashMap<String, Vec<String>> {
        ...
        features.insert(
            "9,10,11".to_string(),
            vec!["default_pie".to_string(), "gnu_unique_object".to_string()],
        );
    ...
    fn init_feature_options() -> HashMap<String, String> {
        ...
        options.insert(
            "gnu_unique_object".to_string(),
            "--enable-gnu-unique-object".to_string(),
        );
        options.insert(
            "link_serialization".to_string(),
            "--with-link-serialization=2".to_string(),
        );
        options.insert("cet".to_string(), "--enable-cet".to_string());
        options.insert(
            "multilib_enable".to_string(),
            "--enable-multilib".to_string(),
        );
        options.insert(
            "multilib_disable".to_string(),
    ...
    pub fn get_gcc_configure_options(
        ...
        // Add base options (let GCC handle ISL detection naturally)
        for option in self.base_options.values() {
        ...
        debug!(
            "Generated {} configure options for GCC {}",
            options.len(),
            gcc_version
        );
        Ok(options)
    ...
    fn detect_cuda_support(&self) -> GccResult<Option<String>> {
        ...
        let nvcc_check = std::process::Command::new("which").arg("nvcc").output();
    ...
    fn get_cuda_directory(&self, nvcc_path: &str) -> Option<String> {
        ...
        nvcc_path
            .rfind("/bin/nvcc")
            .map(|bin_pos| nvcc_path[..bin_pos].to_string())
    }
    ...
    pub fn validate_configure_options(
        ...
        for required in &required_options {
            if !options.iter().any(|opt| opt.starts_with(required)) {
                return Err(GccBuildError::configuration(format!(
                    "Missing required configure option: {}",
                    required
                )));
            }
        }
        ...
        info!(
            "Configure options validation passed for GCC {}",
            gcc_version
        );
        Ok(())
    }
...
pub fn get_configure_options(gcc_version: &GccVersion, config: &Config) -> GccResult<Vec<String>> {
