...
build_pkgs_fn()
{
    ...
    pkgs=(ant autoconf autogen automake binutils bison build-essential ccache ccdiff checkinstall clang \
          ...
          liblzma-dev liblzo2-dev libmimalloc-dev libmusicbrainz5-dev libncurses5-dev libnet-nslookup-perl \
          ...
          meson nano ninja-build openjdk-17-jdk pkg-config python3 python3-pip ragel scons sox texi2html texinfo yasm)
...
if build 'libwebp' 'git'; then
    ...
    execute autoreconf -fi
    execute cmake -S . -DCMAKE_INSTALL_MANDIR:PATH="" -DCMAKE_C_FLAGS_RELEASE:STRING="-O3 -DNDEBUG" -DWEBP_BUILD_EXTRAS:BOOL="0" \
        -DWEBP_BUILD_LIBWEBPMUX:BOOL="0" -DZLIB_LIBRARY_DEBUG:FILEPATH="ZLIB_LIBRARY_DEBUG-NOTFOUND" -DCMAKE_INSTALL_INCLUDEDIR:PATH="include" \
        -DCMAKE_INSTALL_LOCALEDIR:PATH="" -DWEBP_LINK_STATIC:BOOL="0" -DWEBP_BUILD_GIF2WEBP:BOOL="0" -DWEBP_BUILD_IMG2WEBP:BOOL="0" \
        -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL="1" -DWEBP_BUILD_DWEBP:BOOL="0" -DWEBP_BUILD_CWEBP:BOOL="0" -DWEBP_BUILD_ANIM_UTILS:BOOL="0" \
        -DWEBP_BUILD_WEBPMUX:BOOL="0" -DCMAKE_C_FLAGS:STRING="" -DWEBP_ENABLE_SWAP_16BIT_CSP:BOOL="0" -DCMAKE_SHARED_LINKER_FLAGS_RELEASE:STRING="" \
