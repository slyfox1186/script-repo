...
if build 'svtav1' "$g_ver"; then
    ...
    execute cmake -B 'Build/linux' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_BUILD_TYPE='Release' -G 'Ninja' -Wno-dev ../..
...
if build 'opus' "$g_ver"; then
    ...
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_C_FLAGS_DEBUG='-g' \
        -DCPACK_SOURCE_ZIP='1' -G 'Ninja' -Wno-dev
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'libogg' "$g_ver"; then
    ...
    execute autoreconf -fi
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace"  -DCMAKE_BUILD_TYPE='Release' -DBUILD_SHARED_LIBS='0' \
        -DCPACK_BINARY_DEB='1' -DBUILD_TESTING='0'-DCPACK_SOURCE_ZIP='1' -DBUILD_SHARED_LIBS='OFF' -G 'Ninja' -Wno-dev
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'libvorbis' "$g_ver"; then
    ...
    make_dir 'build'
    execute autoreconf -fi
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='0' -G 'Ninja' -Wno-dev
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'frei0r' "$g_ver"; then
    ...
    execute cmake -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DWITHOUT_OPENCV='OFF' \
        ...
        -DWEBP_ENABLE_SWAP_16BIT_CSP='ON' -G 'Ninja' -Wno-dev
    execute ninja "-j$cpu_threads" -C 'build'
    execute ninja "-j$cpu_threads" -C 'build' install
...
if build 'avisynth' "$g_ver"; then
    ...
    execute cmake -S . -DCMAKE_INSTALL_PREFIX="$workspace" -G 'Ninja' -Wno-dev
    execute ninja "-j$cpu_threads"
...
if $nonfree_and_gpl; then
    ...
    if build 'srt' "$g_ver"; then
        ...
        execute cmake -B 'build' -DNATIVE:BOOL='1' -DCMAKE_INSTALL_PREFIX:PATH="$workspace" -DBUILD_APP:BOOL='0' -DYASM_EXE:FILEPATH='/usr/bin/yasm' \
            -DBUILD_SHARED_LIBS='ON' -DCOMPILE_AS_CPP:BOOL='1' -G 'Ninja' -Wno-dev
        execute ninja "-j$cpu_threads" -C 'build' "-j$cpu_threads"
        execute ninja "-j$cpu_threads" -C 'build' install
