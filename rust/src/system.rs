...
impl SystemUtilities {
        ...
        match info_type {
            SystemInfoType::RamMb => {
                let info = sys_info::mem_info()
                    .map_err(|e| GccBuildError::system_requirements(format!("Failed to get memory info: {}", e)))?;
            ...
            SystemInfoType::CpuCores => {
                let cores = sys_info::cpu_num()
                    .map_err(|e| GccBuildError::system_requirements(format!("Failed to get CPU info: {}", e)))?;
                Ok(cores.to_string())
            }
            SystemInfoType::CpuThreads => {
                ...
                match fs::read_to_string("/proc/cpuinfo") {
                    Ok(content) => {
                        let thread_count = content.lines()
    ...
    pub async fn calculate_build_settings(&self, gcc_version_count: usize) -> GccResult<BuildSettings> {
        let available_ram_mb = self.get_system_info(SystemInfoType::AvailableRamMb).await?
            .parse::<u64>()
            .unwrap_or(2000);
        
        let available_cores = self.get_system_info(SystemInfoType::CpuCores).await?
            .parse::<usize>()
            .unwrap_or(2);
        ...
        let required_disk_gb = gcc_version_count as u64 * self.requirements.gb_per_gcc_version + self.requirements.safety_margin_gb;
    ...
    pub async fn validate_requirements(&self, config: &Config) -> GccResult<()> {
        ...
        // Report errors
        if !errors.is_empty() {
            let error_msg = format!("System requirement validation failed:\n{}", 
                                   errors.iter().map(|e| format!("  - {}", e)).collect::<Vec<_>>().join("\n"));
            return Err(GccBuildError::system_requirements(error_msg));
        }
        
        info!("System requirements validation passed");
        info!("Available RAM: {}MB, Available disk: {}GB", available_ram_mb, available_disk_gb);
        Ok(())
    }
    ...
    pub fn get_available_space(&self, path: &Path, unit: SpaceUnit) -> GccResult<u64> {
        #[cfg(unix)]
        {
            ...
            let path_cstring = CString::new(path.to_string_lossy().as_bytes())
                .map_err(|e| GccBuildError::system_requirements(format!("Path conversion error: {}", e)))?;
            
            let mut statvfs: libc::statvfs = unsafe { mem::zeroed() };
            
            let result = unsafe {
                libc::statvfs(path_cstring.as_ptr(), &mut statvfs)
            };
            
            if result != 0 {
                return Err(GccBuildError::system_requirements(
                    format!("Failed to get filesystem stats for {}", path.display())
                ));
            }
        ...
        #[cfg(not(unix))]
        {
            ...
            let output = Command::new("df")
                ...
                .arg(path)
                .output()
                .map_err(|e| GccBuildError::system_requirements(format!("df command error: {}", e)))?;
            
            if !output.status.success() {
                return Err(GccBuildError::system_requirements("df command failed".to_string()));
            }
            ...
            if lines.len() < 2 {
                return Err(GccBuildError::system_requirements("Unexpected df output".to_string()));
            }
            
            let fields: Vec<&str> = lines[1].split_whitespace().collect();
            if fields.len() < 4 {
                return Err(GccBuildError::system_requirements("Cannot parse df output".to_string()));
            }
            
            let available_bytes: u64 = fields[3].parse()
                .map_err(|e| GccBuildError::system_requirements(format!("Cannot parse available bytes: {}", e)))?;
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let ram_usage = match sys_info::mem_info() {
                    Ok(info) => {
                        let usage_percent = (info.total - info.avail) as f64 / info.total as f64 * 100.0;
                ...
                let load_avg = match fs::read_to_string("/proc/loadavg") {
                    Ok(content) => {
                        content.split_whitespace().next().unwrap_or("N/A").to_string()
                    }
                    Err(_) => "N/A".to_string(),
                };
                
                let log_entry = format!("{} RAM:{} DISK:{} LOAD:{}\n", 
                ...
                if let Ok(available_gb) = system_utils.get_available_space(&build_dir, SpaceUnit::GB) {
                    if available_gb < 2 {
