...
# Github Script: https://github.com/slyfox1186/script-repo/blob/main/Bash/Installer%20Scripts/GNU%20Software/build-nano.sh
...
required_packages() {
    ...
    pkgs=(
        ...
        libc6-dev libintl-perl libncurses5-dev libpth-dev
        libticonv-dev libtool libtool-bin lzip lzma-dev nasm
        texinfo
    )
...
configure_build() {
    ...
    autoreconf -fi
    cd build || exit 1
    ../configure --enable-threads=posix --disable-nls --enable-utf8 --enable-year2038 \
                 --with-libpth-prefix=/usr --with-libiconv-prefix=/usr --with-libintl-prefix=/usr
}
...
uninstall_nano() {
    local nano_dir
    nano_dir="$install_dir/$archive_name"
    if [[ -d "$nano_dir" ]]; then
        log "Uninstalling $prog_name from $nano_dir"
        sudo rm -rf "$nano_dir"
