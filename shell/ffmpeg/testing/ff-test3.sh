...
git_1_fn()
{
    ...
    if [ "$github_url" = 'releases/latest' ]; then
        # Latest token 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
...
if build 'yasm' "$g_ver"; then
    ...
    make_dir 'build'
    execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DUSE_OMP='OFF' -G 'Ninja'
...
if build 'googletest' "$g_ver"; then
    ...
    make_dir 'build'
    execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_GMOCK='OFF' -G 'Ninja'
    execute ninja -C 'build'
    execute ninja -C 'build' install
...
if build 'abseil' 'git'; then
    ...
    make_dir 'build'
    execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DCMAKE_EXPORT_COMPILE_COMMANDS='ON' -DABSL_PROPAGATE_CXX_STD='ON' -G 'Ninja'
...
if $nonfree_and_gpl; then
    ...
    if build 'x265' "$g_sver"; then
        ...
        execute cmake -S ../../../source -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_REQUIRED_LIBRARIES='numa' \
...
if build 'SVT-HEVC' "$g_ver"; then
    ...
    make_dir 'build'
    execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_BUILD_TYPE='Release' -G 'Ninja'
    execute ninja -C 'build'
    execute ninja -C 'build' install
    build_done 'SVT-HEVC' "$g_ver"
fi

pre_check_ver 'webmproject/libvpx' '1' 'Ts'
...
if $nonfree_and_gpl; then
    ...
    if build 'vid_stab' "$g_ver"; then
        ...
        make_dir 'build'
        execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_BUILD_TYPE='Release' \
             -DUSE_OMP='ON' -G 'Ninja'
        execute ninja -C 'build'
        execute ninja -C 'build' install
...
if build 'opus' "$g_ver"; then
    ...
    execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='OFF' -DCMAKE_C_FLAGS_DEBUG='-g' \
        -DCPACK_SOURCE_ZIP='1' -G 'Ninja'
    execute ninja -C 'build'
    execute ninja -C 'build' install
...
if build 'libvorbis' "$g_ver"; then
    ...
    make_dir 'build'
    execute autoreconf -fi
    execute cmake -S . -B 'build' -DCMAKE_INSTALL_PREFIX="$workspace" -DBUILD_SHARED_LIBS='0' -G 'Ninja'
    execute ninja -C 'build'
    execute ninja -C 'build' install
...
if build 'avisynth' "$g_ver"; then
    ...
    execute cmake -S . -DCMAKE_INSTALL_PREFIX="$workspace" -G 'Ninja'
    execute ninja
...
if build 'MediaInfoLib' "$g_ver"; then
    ...
    execute cmake . -DCMAKE_INSTALL_PREFIX="$workspace" -DCMAKE_INSTALL_LIBDIR='lib' -DCMAKE_INSTALL_BINDIR='bin' \
        -DCMAKE_INSTALL_INCLUDEDIR='include' -DBUILD_SHARED_LIBS='OFF' -DENABLE_APPS='OFF' \
