...
impl BuildStateManager {
    ...
    pub async fn save_state(&self, state: &BuildState) -> GccResult<()> {
        ...
        tokio::fs::write(&state_file, content).await.map_err(|e| {
            GccBuildError::file_operation(
                "write state file",
                state_file.display().to_string(),
                e.to_string(),
    ...
    pub async fn can_resume(&self, version: &GccVersion, config_hash: &str) -> GccResult<bool> {
        if let Some(state) = self.load_state(version).await? {
            ...
            if state.config_hash != config_hash {
                info!(
                    "Configuration changed, cannot resume build for GCC {}",
                    version
                );
                return Ok(false);
            }
            ...
            if !state.build_dir.exists() {
                info!(
                    "Build directory missing, cannot resume build for GCC {}",
                    version
                );
                return Ok(false);
            }
            ...
            for (phase, phase_state) in &state.phases {
                if phase_state.status == PhaseStatus::Failed {
                    info!(
                        "Phase {:?} failed, can resume from this point for GCC {}",
                        phase, version
                    );
                    return Ok(true);
                }
    ...
    pub async fn cleanup_state(&self, version: &GccVersion) -> GccResult<()> {
        ...
        if state_file.exists() {
            tokio::fs::remove_file(&state_file).await.map_err(|e| {
                GccBuildError::file_operation(
                    "remove state file",
                    state_file.display().to_string(),
                    e.to_string(),
