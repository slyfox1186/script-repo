...
##  Purpose: Build LLVM Clang-18 with GOLD enabled
##  Updated: 03.16.24
##  Script version: 1.1
...
check_root() {
    if [ "$EUID" -ne 0 ]; then
        echo "You must run this script with root/sudo privileges."
        echo
        exit 1
    fi
}
...
# Function to build and install LLVM Clang-18
build_and_install_llvm_clang() {
    local llvm_url="https://github.com/llvm/llvm-project.git"
    local get_latest_version=$(curl -sH "Content-Type: text/plain" "https://raw.githubusercontent.com/slyfox1186/script-repo/main/Bash/Misc/source-git-repo-version.sh" | bash -s "$llvm_url" )
    ...
    local llvm_tar_url="https://github.com/llvm/llvm-project/archive/refs/tags/llvmorg-$get_latest_version.tar.gz"
    local llvm_tar_filename="$llvm_source_file_dir/llvmorg-$get_latest_version.tar.gz"
    ...
    echo
    if build "llvm" "$get_latest_version"; then
...
show_installed_clang_version() {
    local clang_versions=("clang-18" "clang++-18" "clang++")
...
source_files_dir="$PWD/clang-18-build-script"
...
# Build and install LLVM Clang-18
build_and_install_llvm_clang
