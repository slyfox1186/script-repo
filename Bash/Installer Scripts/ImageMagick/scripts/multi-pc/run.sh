...
if [[ ! -f "$PYTHON_ENV_LOCAL" ]]; then
    ...
    if venv -c; then
        ...
        echo "Error: Failed to create the virtual environment locally."
        exit 1
    fi
fi
...
if ! ssh "$REMOTE_SSH" "[ -f $PYTHON_ENV_REMOTE ]"; then
    ...
    if ssh "$REMOTE_SSH" "venv -c"; then
        ssh "$REMOTE_SSH" "venv -p"
    else
...
"$PYTHON_ENV_LOCAL" "$DISTRIBUTE_SCRIPT" "$IMAGE_DIR" "$REMOTE_DIR" "$REMOTE_USER" "$REMOTE_IP" "$(dirname "$0")"
...
if rsync -avz --remove-source-files "$REMOTE_SSH:$REMOTE_DIR/" "$IMAGE_DIR/"; then
