...
if ! $create_service_only; then
    ...
    reflector_cmd="reflector --latest $num_mirrors --sort rate --save /etc/pacman.d/mirrorlist.new"
    ...
    if $dry_run; then
        ...
        if ! eval "$reflector_cmd" 2>&1 | tee -a "$log_file"; then
            ...
            if ! $dry_run; then
                ...
                # Create a new mirrorlist with the header comments and mirror URLs
                {
                    echo "# Updated pacman mirrorlist"
                    echo "# Generated by reflector on $(date '+%Y-%m-%d %H:%M')"
                    echo "# Country: $country"
                    echo "# Protocol: $protocol"
                    cat /etc/pacman.d/mirrorlist.new
                } > /etc/pacman.d/mirrorlist

                # Remove the temporary mirrorlist file
