...
# Github Script: https://github.com/slyfox1186/script-repo/blob/main/Bash/Installer-Scripts/GNU-Software/build-sed.sh
...
uninstall_sed() {
    local sed_dir
    sed_dir="$install_dir/$archive_name"
    if [[ -d "$sed_dir" ]]; then
        log "Uninstalling $prog_name from $sed_dir"
        sudo rm -rf "$sed_dir"
