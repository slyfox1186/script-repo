...
impl AutoTuner {
    pub async fn new() -> GccResult<Self> {
        ...
        info!("🎯 Auto-tuner initialized for {} cores, {:.1}GB RAM, {:?} storage",
              system_profile.cpu_cores,
              system_profile.total_memory_gb,
              system_profile.disk_type);
    ...
    pub async fn optimize_config(
        ...
        let optimized = self.generate_optimized_config(gcc_version, base_config).await?;
        
        // Cache the result
        self.optimization_cache.insert(cache_key, optimized.clone());
        
        info!("✅ Optimized: {} jobs, {} optimization, {:.1}GB memory limit",
    ...
    async fn generate_optimized_config(
        ...
        let parallel_jobs = self.optimize_parallel_jobs(gcc_version, &mut rationale).await?;
        
        // Optimize memory settings
        let memory_limit_mb = self.optimize_memory_limit(parallel_jobs, &mut rationale).await;
        
        // Optimize compilation flags
        let (optimization_level, configure_args) = self.optimize_compilation_flags(
            gcc_version, 
            base_config, 
            &mut rationale
        ).await?;
        ...
        let environment_vars = self.optimize_environment(gcc_version, &mut rationale).await?;
    ...
    async fn optimize_parallel_jobs(
        ...
        if let Some(adjustments) = rules.gcc_rules.version_specific_adjustments.get(&gcc_version.major.to_string()) {
            let factor = adjustments.memory_overhead_factor;
            if factor > 1.0 {
                jobs = ((jobs as f64) / factor).ceil() as usize;
                rationale.push(format!("GCC {} overhead adjustment: -{:.0}%", gcc_version.major, (factor - 1.0) * 100.0));
            }
        }
        ...
        if profile.system_load > 1.0 {
            ...
            rationale.push(format!("High load adjustment: -{:.0}%", (1.0 - adjustment) * 100.0));
        }
    ...
    async fn optimize_memory_limit(&self, parallel_jobs: usize, rationale: &mut Vec<String>) -> Option<u64> {
        let profile = &self.system_profile;
        let rules = &self.tuning_rules;
    ...
    /// Optimize make arguments
    async fn optimize_make_args(&self, parallel_jobs: usize, rationale: &mut Vec<String>) -> Vec<String> {
    ...
    async fn optimize_environment(
        ...
        if gcc_version.major >= 11 {
            env_vars.insert("GCC_COLORS".to_string(), "error=01;31:warning=01;35:note=01;36:caret=01;32:locus=01:quote=01".to_string());
    ...
    async fn optimize_temp_directory(&self, rationale: &mut Vec<String>) -> GccResult<Option<PathBuf>> {
        let profile = &self.system_profile;
    ...
    async fn detect_disk_type() -> GccResult<DiskInfo> {
        ...
        if let Ok(statvfs) = nix::sys::statvfs::statvfs("/") {
            available_gb = (statvfs.blocks() * statvfs.fragment_size() as u64) as f64 / 1_000_000_000.0;
        }
        ...
        Ok(DiskInfo { disk_type, available_gb })
    }
    ...
    fn create_tuning_rules() -> TuningRules {
        ...
        optimization_presets.insert("O0".to_string(), OptimizationPreset {
            base_flags: vec!["-O0".to_string()],
            memory_multiplier: 0.8,
            cpu_multiplier: 1.2,
        });
        
        optimization_presets.insert("O2".to_string(), OptimizationPreset {
            base_flags: vec!["-O2".to_string()],
            memory_multiplier: 1.0,
            cpu_multiplier: 1.0,
        });
        
        optimization_presets.insert("O3".to_string(), OptimizationPreset {
        ...
        version_adjustments.insert("13".to_string(), VersionAdjustments {
            memory_overhead_factor: 1.2,
            compilation_time_factor: 1.1,
            special_flags: vec!["--disable-werror".to_string()],
        });
        
        version_adjustments.insert("14".to_string(), VersionAdjustments {
    ...
    pub async fn get_recommendations(&self, gcc_version: &GccVersion) -> Vec<String> {
        ...
        if profile.total_memory_gb < 8.0 {
            recommendations.push("⚠️ Low memory detected - consider using --preset minimal".to_string());
        }
        ...
        if profile.cpu_cores <= 2 {
            ...
            recommendations.push("🚀 High-end CPU detected - parallel builds will be very efficient".to_string());
        }
        
        // Disk recommendations
        match profile.disk_type {
            DiskType::HDD => {
                recommendations.push("💽 HDD storage detected - builds will be I/O bound".to_string());
            }
            DiskType::SSD => {
                recommendations.push("⚡ SSD storage detected - good build performance expected".to_string());
            }
            DiskType::NVMe => {
                recommendations.push("🏎️ NVMe storage detected - optimal build performance".to_string());
            }
            DiskType::Unknown => {
